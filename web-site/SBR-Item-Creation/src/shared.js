...
const optimize = (text, type) => {
    ...
    for (const part of parts) {
        ...
        if (colors[color] != null) {
            if(prevColor) output += `</span>`;
            output += `<span style=\"color: ${colors[color]}\">${content}`;
            prevColor = true;
        } else if (color == "l") {
            output += `<b>${content}</b>`;
        } else if (color == "k") {
            for (const letter of content.split("")) { output += recombtext(); };
        } else if (color == "o") {
            output += `<i>${content}</i>`;
        } else if (color == "n") {
            output += `<u>${content}</u>`;
...
const recombtext = () => {
    let jumble = "1234567890abcdefghijklmnopqrstuvwxyz~!@#$%^*()-=_+{}[]ABCDEFGHIJKLMNOPQRSTUVWXYZ".split("");
    return jumble[Math.floor(Math.random() * jumble.length)];
}
setInterval(()=>{
    const ID = document.getElementById("itemIDField").value;
    const skull = document.getElementById("skullIDField");
    if(ID == 397){
        skull.readOnly = false;
    }else{
        skull.readOnly = true;
        skull.value = "";
    }
